...
A web interface for the best-performing model (**CamelBERT**) is deployed on Hugging Face Spaces, so the project can be tried out directly in the browser — no setup required.

🔗 **Try it live:** [layaniff/arabic-sentiment](https://huggingface.co/spaces/layaniff/arabic-sentiment)

The interface accepts Arabic text in any of the three dialects and returns the predicted sentiment, detected dialect, and a confidence score — along with quick example phrases for each dialect.

![Web demo screenshot](images/web_demo_screenshot.png)
